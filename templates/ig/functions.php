...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    /**
     * Decorates an article or block with the diablodesign-post style.
     *
    ...
    function artxPost($data)
    {
        ...
        $classes = isset($data['classes']) && strlen($data['classes']) ? $data['classes'] : '';
                    artxFragmentBegin("<article class=\"diablodesign-post" . $classes . "\">");
            artxFragmentBegin("<h2 class=\"diablodesign-postheader\">");
            ...
            artxFragmentEnd("</h2>");
            artxFragmentBegin("<div class=\"diablodesign-postheadericons diablodesign-metadata-icons\">");
            if (isset($data['metadata-header-icons']) && count($data['metadata-header-icons']))
                foreach ($data['metadata-header-icons'] as $icon)
                    artxFragment('', $icon, '', ' | ');
            artxFragmentEnd("</div>");
            artxFragmentBegin("<div class=\"diablodesign-postcontent clearfix\">");
            if (isset($data['content']) && strlen($data['content']))
                artxFragmentContent(artxPostprocessPostContent($data['content']));
            artxFragmentEnd("</div>");
            artxFragmentBegin("<div class=\"diablodesign-postmetadatafooter\">");
            artxFragmentBegin("<div class=\"diablodesign-postfootericons diablodesign-metadata-icons\">");
    ...
    function artxBlock($caption, $content, $classes = '')
    {
        ...
        if (!$hasCaption && !$hasContent)
            return '';
                    artxFragmentBegin("<div class=\"diablodesign-block clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"diablodesign-blockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"diablodesign-blockcontent\">");
    ...
    function artxVMenuBlock($caption, $content, $classes = '')
{
    ...
    if (!$hasCaption && !$hasContent)
        return '';
                artxFragmentBegin("<div class=\"diablodesign-vmenublock clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"diablodesign-vmenublockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"diablodesign-vmenublockcontent\">");
            artxFragmentContent(artxPostprocessBlockContent($content));
            artxFragmentEnd("</div>");
     ...
     *  Select inputs with class != 'diablodesign-button'
     *   artxFindTags($html, array('input' => array('class' => '!diablodesign-button')))
     *  Select inputs with non-empty class
     *   artxFindTags($html, array('input' => array('class' => '!')))
     *  Select inputs with class != 'diablodesign-button' and non-empty class:
     *   artxFindTags($html, array('input' => array('class' => array('!diablodesign-button', '!'))))
     *  Select inputs with class = button but != 'diablodesign-button'
     *   artxFindTags($html, array('input' => array('class' => array('button', '!diablodesign-button'))))
     *
    ...
    function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
    {
        ...
        foreach (artxFindTags($content, $filter) as $tag) {
            if (is_string($tag))
                ...
                $tag['attributes']['class']['value'][] = 'diablodesign-button';
    ...
    function artxLinkButton($data = array())
    {
        return '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
            . 'diablodesign-button" href="' . $data['link'] . '">' . $data['content'] . '</a>';
    }
    ...
    {
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!diablodesign-button')),
            'button' => array('class' => array('button', '!diablodesign-button'))));
        return $content;
    }
}
