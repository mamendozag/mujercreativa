...
if (isset($attribs['name']) && $attribs['name'] == 'position-1') {
    ...
    echo '<ul class="diablodesign-hmenu"' . $tag . '>';
    ...
    echo '</ul>';
} else if (0 === strpos($params->get('moduleclass_sfx'), 'diablodesign-vmenu') || false !== strpos($params->get('moduleclass_sfx'), ' diablodesign-vmenu')) {
    $menutype = 'vertical';
    ...
    echo '<ul class="diablodesign-vmenu"' . $tag . '>';
    foreach ($list as $i => & $item) {
        if ($skip) {
            if ($item->shallower) {
                if (($item->level - $item->level_diff) <= $limit) {
