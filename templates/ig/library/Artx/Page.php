...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="diablodesign-content-layout">';
        $result .= '<div class="diablodesign-content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="diablodesign-layout-cell' . ('diablodesign-block' == $style ? ' diablodesign-layout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="diablodesign-post">' string in it.
     * Then it wraps the content of the buffer with the diablodesign-post.
     */
    public function componentWrapper()
    {
        ...
        if ('com_content' == $option && ('featured' == $view || 'article' == $view || ('category' == $view && 'blog' == $layout)))
            return;
        if (false === strpos($content, '<div class="diablodesign-post'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'diablodesign-main').addClass('diablodesign-postcontent');
            }
        });
    }
});
